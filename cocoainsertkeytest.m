...
- (void)ev:(NSEvent *)e
{
	...
	if (sc == nil)
		sc = @"";
	scim = [e charactersIgnoringModifiers];
	if (scim == nil)
		scim = @"";
	c = [sc dataUsingEncoding:NSUTF32StringEncoding];
	cim = [scim dataUsingEncoding:NSUTF32StringEncoding];
	printf("k:%X c:[", [e keyCode]);
	b = (uint32_t *) [c bytes];
	...
	printf("] i:[");
	b = (uint32_t *) [cim bytes];
	for (i = 0; i < [cim length]; i += 4) {
		if (i != 0)
			printf(" ");
		printf("%X", b[i / 4]);
	}
...
#define ADD(c) [[mainwin contentView] addSubview:(c)]
	...
	qv *v = [[qv alloc] initWithFrame:[[mainwin contentView] frame]];
	RESIZE(v, NSViewWidthSizable | NSViewHeightSizable);
	ADD(v);
}
