...
#define ADDNEXTTO(widget, nextto, side, xspan, yspan) gtk_grid_attach_next_to(grid, (GtkWidget *) widget, (GtkWidget *) (nextto), GTK_POS_ ## side, (xspan), (yspan))
	...
	EXPAND(widget2, TRUE, FALSE);
	ALIGN(widget2, FILL, START);
	ADDNEXTTO(widget2, prev, RIGHT, 1, 1);
	widget = gtk_entry_new();
	EXPAND(widget, TRUE, FALSE);
	ALIGN(widget, FILL, START);
	ADDNEXTTO(widget, widget2, BOTTOM, 1, 1);
	widget2 = gtk_entry_new();
	EXPAND(widget2, TRUE, FALSE);
	ALIGN(widget2, FILL, START);
	ADDNEXTTO(widget2, widget, BOTTOM, 1, 1);
	widget = gtk_entry_new();
	EXPAND(widget, TRUE, FALSE);
	ALIGN(widget, FILL, START);
	ADDNEXTTO(widget, widget2, BOTTOM, 1, 1);
	widget2 = gtk_button_new_with_label("Draw");
	EXPAND(widget2, TRUE, TRUE);
	ALIGN(widget2, FILL, FILL);
	ADDNEXTTO(widget2, widget, BOTTOM, 1, 1);
	widget = gtk_button_new_with_label("All");
	EXPAND(widget, TRUE, FALSE);
	ALIGN(widget, CENTER, START);
	ADDNEXTTO(widget, widget2, BOTTOM, 1, 1);
	widget = gtk_label_new("Test");
	EXPAND(widget, FALSE, FALSE);
	ALIGN(widget, START, START);
	ADDNEXTTO(widget, widget2, RIGHT, 1, 1);
