...
@implementation NSThread (FakeMainThread)

+ (NSThread *)fakeMainThread
{
	return mainthread;
}
...
void replaceClassImpl(Class class, SEL old, SEL new)
{
	Method oldm, newm;
	Class metaclass;

	oldm = class_getClassMethod(class, old);
	newm = class_getClassMethod(class, new);
	metaclass = object_getClass((id) class);
	// ignore that I'm doing an explicit compare to YES here
	if (class_addMethod(metaclass, old,
		method_getImplementation(newm),
		method_getTypeEncoding(newm)) == YES)
		class_replaceMethod(metaclass, new,
			method_getImplementation(oldm),
			method_getTypeEncoding(oldm));
	else
		method_exchangeImplementations(oldm, newm);
}

void replaceInstanceImpl(Class class, SEL old, SEL new)
{
	Method oldm, newm;

	oldm = class_getInstanceMethod(class, old);
	newm = class_getInstanceMethod(class, new);
	// ignore that I'm doing an explicit compare to YES here
	if (class_addMethod(class, old,
		method_getImplementation(newm),
		method_getTypeEncoding(newm)) == YES)
		class_replaceMethod(class, new,
			method_getImplementation(oldm),
			method_getTypeEncoding(oldm));
	else
		method_exchangeImplementations(oldm, newm);
...
{
	replaceClassImpl([NSThread class], @selector(mainThread), @selector(fakeMainThread));
	replaceClassImpl([NSThread class], @selector(isMainThread), @selector(fakeIsMainThread));
	replaceInstanceImpl([NSThread class], @selector(isMainThread), @selector(fakeIsMainThread));
	init(argc, argv);
...
- (NSApplicationTerminateReply)applicationShouldTerminate:(NSApplication *)x
{
