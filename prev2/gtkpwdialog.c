...
int main(int argc, char *argv[])
{
	...
	GtkWidget *m;
#define M GTK_MESSAGE_DIALOG(m)

	m = gtk_message_dialog_new(mainwin, GTK_DIALOG_MODAL,
	...
	gtk_message_dialog_set_image(M, icon);

	GtkBox *box;
	GtkWidget *entry;

	box = GTK_BOX(gtk_message_dialog_get_message_area(M));
	entry = gtk_entry_new();
	gtk_entry_set_visibility(GTK_ENTRY(entry), FALSE);
	gtk_box_pack_end(box, entry, TRUE, TRUE, 0);
