...
char *args = "{long|short|shortcentury|time|fmt format}";		// other command-line arguments here, if any
...
BOOL parseArgs(int argc, char *argv[])
{
	...
	prevdtpstyle = dtpstyle;			// save to detect if it was changed
	if (optind >= argc)				// no args
		return FALSE;
	mode = argv[optind];
	// TODO ensure correct argument count
	if (strcmp(mode, "long") == 0)
		dtpstyle |= DTS_LONGDATEFORMAT;
	else if (strcmp(mode, "short") == 0)
		dtpstyle |= DTS_SHORTDATEFORMAT;
	else if (strcmp(mode, "shortcentury") == 0)
		dtpstyle |= DTS_SHORTDATECENTURYFORMAT;
	else if (strcmp(mode, "time") == 0)
		dtpstyle |= DTS_TIMEFORMAT;
	else if (strcmp(mode, "fmt") == 0)
		dtpfmt = argv[optind + 1];
	else
		goto fail;
	return TRUE;
...
#define SETFONT(hwnd) SendMessage(hwnd, WM_SETFONT, (WPARAM) controlfont, (LPARAM) TRUE);
	...
	dtp = CreateWindowEx(CXSTYLE,
		DATETIMEPICK_CLASS, L"",
		dtpstyle | CSTYLE,
		20, 20, 250, 30,
		mainwin, NULL, hInstance, NULL);
	if (dtp == NULL)
		panic("error making date-time picker");
	SETFONT(dtp);
	if (dtpfmt != NULL) {
		TCHAR *wdtpfmt;
		int n;

		n = MultiByteToWideChar(CP_UTF8, 0, dtpfmt, strlen(dtpfmt), NULL, 0);
		if (n == 0)
			panic("failed getting number of bytes to convert \"%s\" to wide-character string", dtpfmt);
		wdtpfmt = (TCHAR *) malloc((n + 1) * sizeof (TCHAR));
		if (wdtpfmt == NULL)
			panic("error allocating wide-character string conversion of \"%s\"", dtpfmt);
		if (MultiByteToWideChar(CP_UTF8, 0, dtpfmt, strlen(dtpfmt), wdtpfmt, n) == 0)
			panic("error converting \"%s\" to wide-character string", dtpfmt);
		if (SendMessage(dtp, DTM_SETFORMAT,
			(WPARAM) 0, (LPARAM) wdtpfmt) == 0)
			panic("error setting date-time picker format");
		free(wdtpfmt);
	}
}
