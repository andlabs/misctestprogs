...
#define WINDOWCOLOR COLOR_BTNFACE
...
#define CREATEPEN(colorref) CreatePen(PS_SOLID, 2, colorref);
	pens[0] = CREATEPEN(GetSysColor(WINDOWCOLOR));
#define MKPEN(r,g,b) CREATEPEN(RGB(r, g, b))
#define XC 216
	pens[1] = MKPEN(0, 0, 0);
	pens[2] = MKPEN(XC, 0, 0);
	pens[3] = MKPEN(0, XC, 0);
	pens[4] = MKPEN(0, 0, XC);
	pens[5] = MKPEN(XC, 0, XC);
	return TRUE;
}
...
LRESULT CALLBACK wndproc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	...
	switch (msg) {
#define DOWN(x,n,r) case x:SetWindowLongPtr(hwnd,GWLP_USERDATA,n);start=MAKEPOINTS(lparam);return r;
	DOWN(WM_LBUTTONDOWN, 1, 0)
	DOWN(WM_MBUTTONDOWN, 2, 0)
	DOWN(WM_RBUTTONDOWN, 3, 0)
	DOWN(WM_XBUTTONDOWN, 3 + ((wparam >> 16) & 0xFFFF), TRUE)
#define UP(x,r) DOWN(x,0,r)
	UP(WM_LBUTTONUP, 0) UP(WM_MBUTTONUP, 0)
	UP(WM_RBUTTONUP, 0) UP(WM_XBUTTONUP, TRUE)
	case WM_MOUSEMOVE:
		cur = MAKEPOINTS(lparam);
		InvalidateRect(hwnd, NULL, TRUE);
		return 0;
	case WM_PAINT:
		n = GetWindowLongPtr(hwnd, GWLP_USERDATA);
		dc = BeginPaint(hwnd, &ps);
		prev = SelectObject(dc, pens[n]);
		MoveToEx(dc, start.x, start.y, NULL);
		LineTo(dc, cur.x, cur.y);
...
HWND makeMainWindow(void)
{
	...
	cls.hbrBackground = (HBRUSH) (WINDOWCOLOR + 1);
	if (RegisterClass(&cls) == 0)
		panic("error registering window class");
	hwnd = CreateWindowEx(0,
		L"mainwin", L"Main Window (Note: Does Little Error Checking!)",
		WS_OVERLAPPEDWINDOW,
