...
LRESULT CALLBACK wndproc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	...
	case WM_COMMAND:
		if (LOWORD(wparam) == 100 && HIWORD(wparam) == BN_CLICKED) {
			MessageBox(hwnd, L"test", L"test", hwnd == mainwin2 ? MB_OK : MB_ICONERROR);
			printf("message box for window %d returned\n", hwnd == mainwin2 ? 2 : 1);
		}
		return 0;
	case WM_CLOSE:
		PostQuitMessage(0);
...
#define SETFONT(hwnd) SendMessage(hwnd, WM_SETFONT, (WPARAM) controlfont, (LPARAM) TRUE);
	...
	b = CreateWindowEx(CXSTYLE,
		L"button", L"Click",
		BS_PUSHBUTTON | CSTYLE,
		20, 20, 200, 200,
		mainwin, (HMENU) 100, hInstance, NULL);
	if (b == NULL)
		panic("button creation failed");
	SETFONT(b);
}
...
int main(int argc, char *argv[])
{
	...
	mainwin2 = makeMainWindow();
	buildUI(mainwin2);
	firstShowWindow(mainwin2);
