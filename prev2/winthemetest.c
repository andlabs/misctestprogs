...
#define S(x) { #x , x }
S(CBS_CHECKEDDISABLED),
S(CBS_CHECKEDHOT),
S(CBS_CHECKEDNORMAL),
S(CBS_CHECKEDPRESSED),
S(CBS_UNCHECKEDDISABLED),
S(CBS_UNCHECKEDHOT),
S(CBS_UNCHECKEDNORMAL),
S(CBS_UNCHECKEDPRESSED),
{ NULL, 0 },
};
	...
	case WM_CREATE:
		{
			...
			if (theme == NULL)
				panic("OpenThemeData() failed");
			for (i = 0; states[i].name != NULL; i++) {
				res = GetThemeInt(theme, BP_CHECKBOX, states[i].state, TMT_WIDTH, &w);
				if (res != S_OK)
					panic("error getting width: %d", res);
				res = GetThemeInt(theme, BP_CHECKBOX, states[i].state, TMT_HEIGHT, &h);
				if (res != S_OK)
					panic("error getting height: %d", res);
				printf("checkbox state %s size %d x %d, transparent %d\n",
					states[i].name, w, h,
					IsThemeBackgroundPartiallyTransparent(theme, BP_CHECKBOX, states[i].state));
			}
			exit(0);
		}
		return 0;
